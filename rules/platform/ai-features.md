...
- **AIS-1** A feature counts as an AI feature when the app sends anything to
  ...
  the app keeps the project rules only. [Coast's own scope line; Google
  Play's AI-generated content policy draws a narrower scope for its OWN
  purposes — see STORE]
...
- **DISC-1** Where a person interacts with a model (chat, assistant, voice),
  ...
  hidden bot. [EU AI Act Art. 50(1); NIST AI 600-1]
- **DISC-2** Text, images, audio or video the app generates and shows or
  ...
  [EU AI Act Art. 50(2) — live since 2 August 2026; C2PA 2.4]
- **DISC-3** A generated summary or rewrite of a person's OWN content is
  ...
  the app never relies on the carve-out silently. [EU AI Act Art. 50(2) —
  the carve-out's reach is unsettled; Coast default, veto-able]
- **DISC-4** No screen, listing or marketing copy claims the model can
  ...
  is classified and cleared to make that claim. [Google Play health apps
  policy; EU AI Act Annex III (the claim is the trigger)]
...
- **FLOW-1** Before any personal data leaves the device for a model
  ...
  consent. [Google Play User Data policy, prominent disclosure; Apple App
  Review 5.1.2(i)]
- **FLOW-2** Health data from the platform's health store and clinical data
  ...
  use-based data mining — no consent screen unlocks this. [Apple App Review
  5.1.3; Google Play Health Connect policy]
- **FLOW-3** The data sent to a model is the minimum the feature needs:
  ...
  needed for the answer are stripped before the request. [AISVS v1.0 C2
  Input Validation; LLM02:2026]
- **FLOW-4** Inferring a person's emotion or intention from biometric data
  ...
  rules. [EU AI Act Annex III; Art. 50(3)]
...
- **PROMPT-1** Every prompt is assembled in one place from a named template
  ...
  inline. [AISVS v1.0 C3 Model Lifecycle Management & Change Control;
  Coast DRY rule]
- **PROMPT-2** Content from outside the app — web pages, files, messages,
  ...
  as instructions, and the template says so to the model. [LLM01:2026
  Prompt Injection; AISVS v1.0 C2]
- **PROMPT-3** Secrets, keys, internal URLs and other people's data never
  appear in a prompt or a system instruction. [LLM08:2026 Hidden Context
  Exposure; LLM02:2026]
- **PROMPT-4** Model calls go through the app's own backend or a provider
  proxy; a provider API key is never shipped inside the app. [AISVS v1.0
  C5 Access Control & Identity; OWASP MASVS]
...
- **OUT-1** Model output is treated as untrusted input: it is never
  ...
  [LLM10:2026 Improper Output Handling; AISVS v1.0 C7 Model Behavior,
  Output Control & Safety Assurance]
- **OUT-2** Output that drives a decision with consequences (a payment, a
  ...
  before it takes effect. [LLM03:2026 Excessive Agency]
- **OUT-3** Every place a person sees model output offers a way to report or
  flag it, inside the app, without leaving the screen. [Google Play AI-
  generated content policy — the one hard requirement]
- **OUT-4** Factual claims the model makes are shown with their source when
  ...
  has none. [LLM07:2026 Misinformation]
...
- **AGENT-1** A model may call only the tools the feature declares; each
  ...
  using the app has. [LLM03:2026; AISVS v1.0 C9 Orchestration & Agentic
  Security; OWASP Top 10 for Agentic Applications 2026]
- **AGENT-2** Tools that change state (write, send, pay, delete) require the
  ...
  person granted on screen and can revoke. [LLM03:2026]
- **AGENT-3** An agent loop has a fixed maximum number of steps and a
  timeout; hitting either stops the loop and tells the person. [AISVS v1.0
  C9; LLM06:2026 Unbounded Consumption]
...
- **COST-1** Every model call carries a maximum output length, and every
  ...
  calling. [LLM06:2026 Unbounded Consumption]
- **COST-2** Retries on provider errors are bounded and backed off; a loop
  that retries a model call is a defect. [LLM06:2026]
...
- **REL-1** Every model call has a timeout and a plain-words fallback the
  ...
  spins without end. [AISVS v1.0 C12 Monitoring, Logging & Anomaly
  Detection]
- **REL-2** The app works without the AI feature: a model outage degrades
  one feature, never the whole app. [Coast default, veto-able]
- **REL-3** Provider, model name and version are configuration read in one
  place, so a model change is one diff and one review. [AISVS v1.0 C3]
...
- **EVAL-1** Each AI feature ships with a written set of example inputs and
  ...
  them. [AISVS v1.0 C3; OWASP AI Testing Guide]
- **EVAL-2** Tests of model output assert deterministic properties (shape,
  ...
  they never assert exact generated prose. [Coast default, veto-able — no
  primary source gives a rule for non-determinism]
- **EVAL-3** Known prompt-injection and jailbreak probes for the feature's
  shape are part of the suite and must fail to take control. [LLM01:2026;
  AISVS v1.0 C11 Adversarial Robustness]
...
- **LOG-1** Prompts and outputs are logged only when the person has agreed,
  ...
  before they are stored. [AISVS v1.0 C12; LLM02:2026]
- **LOG-2** Every model call is traceable: a request id, the template
  ...
  prompt text. [AISVS v1.0 C12]
- **LOG-3** Logs of model traffic have a retention period and are deleted
  on schedule. [AISVS v1.0 C12]

## Retrieval (RAG)

- **RAG-1** Documents a feature retrieves for the model are filtered by the
  person's own access rights before retrieval, never after. [LLM09:2026
  Vector and Embedding Weaknesses; AISVS v1.0 C8 Memory, Embeddings &
  Vector Database Security]
- **RAG-2** Retrieved text is untrusted input (see PROMPT-2); a document
  can never instruct the model. [LLM01:2026; LLM09:2026]
- **RAG-3** Embeddings of personal data are stored and deleted under the
  same rules as the data they came from. [AISVS v1.0 C8]
...
- **SUPPLY-1** Model weights, model files and AI libraries are pinned by
  ...
  dependency record. [LLM04:2026 Supply Chain; AISVS v1.0 C6 Supply Chain
  Security for Models]
- **SUPPLY-2** A model or dataset from outside is checked for its licence
  ...
  [LLM04:2026; LLM05:2026 Data and Model Poisoning]
...
- **STORE-1** Before submission, the app's AI features are described
  ...
  data with third-party AI apply). [Google Play Console requirements; Apple
  App Review 4.7, 5.1.2(i)]
- **STORE-2** A chatbot or generated-content feature ships with the report
  ...
  produce. [Google Play AI-generated content policy; Apple App Review 4.7]
...
- **EU-1** Before an AI feature ships, a filed decision records its EU AI
  ...
  transparency duties apply since 2 August 2026; high-risk obligations
  follow from December 2027 / August 2028]
- **EU-2** An app placed on the market before 2 August 2026 completes its
  ...
  day one. [Regulation (EU) 2026/1744]
