...
- **PAY-4** Digital goods and features sold inside the app use the platform's in-app purchase system. [Apple App Review Guidelines §3.1; check: review]
...
- **ACC-2** Tap and click targets are at least 44×44 points, as platform guidance says, and never smaller than 24×24 pixels. [Apple HIG; WCAG 2.2 — 2.5.8; check: review]
...
- **ENG-6** Secrets are kept in the platform's secure store (Keychain on Apple platforms, Keystore on Android), never in plaintext files, source code or logs. [OWASP MASVS-STORAGE; Apple platform security; check: scan:secret-literal, review]
