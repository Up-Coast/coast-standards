...
- **Every change updates the documents that describe it.** When you change a behaviour, command, flag, setting, file location or message, search the project's documents for everything that describes it: the README, guides, settings references, troubleshooting pages, diagrams and the change log. Update them in the same change. A change that leaves a document describing the old behaviour is not finished. [check: review]
- **Documents follow the project's writing instructions.** Write and update documents by the project's documentation instructions: the writing guides installed in `.claude/skills/` (`write-developer-documentation` and `write-a-guide`), or the project's own instructions when it switched those guides off. [check: review]
