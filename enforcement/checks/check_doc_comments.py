...
* **TypeScript / JavaScript** — exported top-level declarations
  (``export [default] [abstract] class | interface | type | enum | function
  | const | let | var``) and, inside an exported class or interface, its
  members that are not ``private``/``protected``/``#name``/``constructor``.
  A doc is a ``/** */`` block directly above (decorators may sit between).
* **Python** — module-level ``def``/``class`` whose name does not start with
  ``_``, and methods of a public class whose name does not start with ``_``
  (``__init__`` included, PEP 257). A doc is a docstring as the first
  statement.
...
PY_DOCSTRING = re.compile(r"""^\s*[rRuUbBfF]{0,2}("{3}|'{3}|"|')""")
...
def python_declarations(source):
    ...
    index = 0
    while index < len(lines):
        match = PY_DEF.match(lines[index])
        if not match:
            index += 1
            continue
        ...
        while first_body < len(lines) and not lines[first_body].strip():
            first_body += 1
        documented = first_body < len(lines) and PY_DOCSTRING.match(lines[first_body]) is not None
        decorated_overload = index > 0 and "@overload" in lines[index - 1]
        declaration = Declaration("class" if kind == "class" else "func", name, "doc" if documented or decorated_overload else "",
                                  not name.startswith("_") or name == "__init__", index + 1)
        if indent == 0:
            out.append(declaration)
            container = declaration if kind == "class" else None
            class_indent = indent
        elif container is not None and class_indent is not None and indent > class_indent \
                and all(len(l) - len(l.lstrip()) > class_indent or not l.strip() for l in lines[container.line:index]):
            container.members.append(declaration)
        index = first_body if first_body > index else index + 1
    return out
