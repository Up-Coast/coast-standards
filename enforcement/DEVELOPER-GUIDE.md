...
| **Signature** | One regex entry in `rules_signatures.json`: an id, a severity, the path classes it applies to, a pattern, and the words it prints. |
...
2. installs the Claude Code session layer (`Scripts/hooks/claude-hook.py`, the `hooks` key
...
7. rewrites the marked block in `CLAUDE.md` with the platform, the standards commit and the
   number;
...
Re-run the same command. Governed files are replaced, seeds you edited are kept with a
...
The whole-tree pass belongs to the push. A commit-time (`--staged`) or editor-time
(`--files`) scan runs the tree-scope `block` signatures over the touched files only and
judges no ratchet, so it takes seconds on a large repository.
      ...
      "signatures": [
        {
          ...
          "words": "a plaintext http:// literal — transport is TLS (SEC-1)"
        }
      ]
    }
  }
...
|---|---|---|
| `id` | yes | the signature's name; a rule's `scan:<id>` tag names it |
| `rule` | yes | the rule id it holds, printed in brackets on every hit |
| `severity` | yes | `block`, `ratchet` or `advisory` |
| `scope` | yes | `added` (only added lines) or `tree` (every file, at push) |
| `applies_to` | yes | path classes the signature runs on; `["*"]` for every class |
| `excludes` | no | path classes it never runs on (`tests`, `strings`, `theme`, …) |
| `files` | no | file globs narrowing it further (`**/*.swift`) |
| `files_excluded` | no | file globs it skips even inside an applying class |
| `pattern` | yes for regex | a Python `re` pattern matched against each line |
| `paired` | no | a second pattern that must also match within the same joined-lines window (native-pattern shape) |
| `kind` | no | `builtin` for signatures implemented in code rather than regex (§5.4) |
| `group` | no | a label such as `native-pattern` or `secret-literal` grouping related ids |
| `once` | no | `true` to report a file once, not per line (`second-theme-file`, `secret-file`) |
| `words` | yes | the sentence printed on a hit — say what is wrong and where it belongs |

Every signature has a plant: `checks/tests/plants/<platform>/<id>.fail.<ext>` must hit and
`<id>.pass.<ext>` must not. `test_check_rules.py` runs every plant in both directions.
...
|---|---|---|
| `ui-string-literal` | `literals.py` | lexes the whole file the way Coast's copy guard did: Swift (with the guard's skip contexts — SF Symbols, identifiers, URLs, comparisons, SQL), Kotlin/XML, TSX/JSX text and known text props, Python message shapes; only added lines are reported |
| `doc-comments` | `check_doc_comments.py` | every public declaration without a doc comment (§6) |
| `import-matrix` | `import_matrix.py` | module-layer imports against `.coast/module-kinds.json` (app / feature / shared) |
...
```json
{
  "baselines": [
    {"id": "spacing-literal", "count": 839, "deadline": "2026-12-03",
     "written": "2026-09-04", "by": "Abbey Jackson", "moves": []},
...
Past the deadline every ratchet becomes `block`: anything above zero refuses. Lowering the
count never moves the date. Only a person moves a deadline, and the move is recorded under
...
```json
{
  "exceptions": [
    {"id": "secret-literal", "path": "Tests/AuthTests/FixtureKeys.swift",
     ...
     "who": "Abbey Jackson", "when": "2026-09-04"}
  ]
}
```
...
| TypeScript | exported declarations and the non-private members of exported classes and interfaces | `/** */` |
| Python | PEP 257: public module-level defs and classes, public methods and `__init__` | a docstring as the first statement |
...
| `rules-scan` | the scanner on the lines added since the remote (starting at the adoption commit on the first push), then `--tree` with the ratchets | same | same | same |
...
no push lock and runs no previous hooks.
...
{"exceptions": [
  {"seat": "build", "reason": "the build needs env vars this checkout has not got",
   "who": "Abbey Jackson", "when": "2026-09-05", "until": "2026-09-19"}
]}
```
...
`.coast/previous-hooks-path`, and each shipped hook runs the previous hook of the same name
after its own checks pass, with the same arguments and the same ref lines on stdin. The
installer also warns when a `package.json` `prepare` script would set the old path back.

---

## 9. The Claude Code session hooks

Committed in `.claude/settings.json` (the `hooks` key only; other keys are the founder's).
One entry point, `Scripts/hooks/claude-hook.py <hook-id>`, reads the event JSON on stdin and
exits 2 with the reason on stderr to refuse. Every behaviour was verified against
Anthropic's hooks reference (https://code.claude.com/docs/en/hooks) and the quotes are in
the file's docstring.

| Hook id | Event | Matcher | Refuses |
|---|---|---|---|
| `governing-edit` | PreToolUse | `Edit\|Write\|MultiEdit\|NotebookEdit` | a path in the platform's `governing` class or `.claude/settings.json` |
| `chained-cd` | PreToolUse | `Bash` | `cd X && …`, `cd X; …`, a `cd` line followed by more; bare `cd` passes |
| `infra-command` | PreToolUse | `Bash` | fly/flyctl, wrangler, cloudflared, terraform/tofu, pulumi, doctl, aws, gcloud, az, nsupdate, `gh repo delete`, `gh secret\|variable\|ruleset`, a mutating `gh api` on repo/ruleset/secret/hook/key/environment/deployment/protection endpoints |
...
Known limit: a user-level `disableAllHooks` turns these off. That is the human's own
machine; the git hooks still stand.
...
`Package.swift` with `.iOS(` only → ios, `.macOS(` only → macos (both or neither: ask);
...
| `.claude/settings.json` → `hooks` | governed key | the session hook wiring; other keys kept |
| `.coast/platform` | governed | one word |
| `.coast/standards-version` | governed | the standards commit adopted |
...
| `.coast/previous-hooks-path` | governed | the hooks path the project had before |
...
| `CLAUDE.md` | the block between `<!-- up-coast-standards: begin -->` / `end -->` | rewritten from `TEMPLATE-PROJECT-CLAUDE.md`; text outside the markers untouched; appended once if no markers |
...
| `eslint.config.mjs` | same | `recommendedTypeChecked` with `projectService`; `react-hooks/rules-of-hooks`, `exhaustive-deps`; `@typescript-eslint/no-floating-promises`; `max-lines` warn |
| `.prettierrc.json` | same | |
| `tsconfig.seed.json` | `tsconfig.json` | `strict`, `noImplicitAny` |
...
- `.coast/standards-version` records the standards commit a project adopted. Re-running the
  installer moves it.
...
root, and `disableAllHooks` in user settings turns them off. Check `.claude/settings.json`
has the `hooks` key and that `Scripts/hooks/claude-hook.py` exists.
...
1. Add the entry to `checks/rules_signatures.json` under every platform the rule applies to.
...
| File | Covers |
|---|---|
| `test_check_rules.py` | every scanner mode on a synthetic git repo, exceptions, the ratchet in both directions and past its deadline, advisory, every plant in both directions |
...
| `test_lint_configs.py` | every config exists and every linter tag the corpus cites is enabled in it |
...
- The Claude Code hooks depend on the session being started at the repository root and on
  hooks not being disabled at the user level.
