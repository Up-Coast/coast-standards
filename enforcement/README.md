...
`~/Claude/Work/Code/tools/coast-standards/enforcement/README.md` and build
...
| Task | What | Size | Guard |
|---|---|---|---|
| E6.1 | **DONE 2026-09-07 (Fable).** `gh repo rename coast-standards` (GitHub's docs, read first: every clone, fetch and push to the old name redirects; never create a new repo under the old name or the redirect dies) and the local folder moved to `~/Claude/Work/Code/tools/coast-standards`, with the per-project memory folder moved to the new path key; the `Up-Coast` org stays. | S | `git fetch` from the new remote; `gh repo view Up-Coast/coast-standards` |
| E6.2 | **DONE 2026-09-07 (Fable).** 22 files rewritten: this repo, `~/.claude/CLAUDE.md`, every memory file, the four adopted repos' CLAUDE.md (uncommitted there until E2.7 re-adopts and commits), Coast's plan documents and `.coast/paths.json`, ccm-replacement's docs, fleetsignal's frontend rules. Left on purpose: transcripts, atlas exports, agent worktree copies, and the CLAUDE.md block markers `<!-- up-coast-standards: begin/end -->`, which are `adopt.py`'s identifier and change with E5.1 (the installer must recognise both spellings for one release). | S | `grep -r up-coast-standards` over `~/.claude` and `~/Claude/Work/Code` finds only history, transcripts and the markers |
