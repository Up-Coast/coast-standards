...
│   │   ├── layout.json/.py       the one table of every path the layer names in a project (E5.1)
│   │   ├── config.default.json   the config's defaults; config.py is its one loader (E5.3)
...
- Reads `rules_signatures.json`: one row per signature, `{id, severity:
  ...
  entry.
- Reads the project's config (`<state dir>/config.json`, through `config.py`; E5.3):
  a signature in `rules.off` is not run, a severity in `rules.severity` is lowered
  (never raised), the `retired_words` join the retired-wording pattern.
...
| E2.7 | **DONE 2026-09-08 for three of the four (release 1.1.0).** The three iOS apps were re-adopted with `adopt.py <repo> --release 1.1.0 --yes`, committed and pushed through their batteries green. The reference implementation was re-adopted and committed, and its push was REFUSED by its own jscpd seat: four clones in `Templates/throwaway/Scripts/checks/` (unchanged since 2026-09-03) were hidden from jscpd by the old `**/Scripts/checks/**` ignore glob, which 1.1.0's layout no longer carries, so they read as new against a baseline written 2026-09-04. Nothing raises a baseline and an agent writes no governing file, so the owner decides: extract the shared piece, add that folder to `layout.jscpd_ignore` in the project's config (it is the vendored copy of this layer, not product code), or excuse the `jscpd` seat with a date. The commit waits locally until then. **Re-adopt the four repositories.** Coast and the three iOS apps carry a standards version five commits behind this review, and more after it: they lack the seat exceptions (§4.7b), the previous-hooks chaining (§4.8), the tsconfig parser, and everything in E2.6. Each also has an uncommitted CLAUDE.md edit (the standards clone moved on the owner's machine) that `adopt.py` will rewrite anyway. Run `adopt.py <repo>` (no `--measure-tools`; the counts stand), commit, push through the gate. Note the `pii-in-log` and `spacing-literal` counts may FALL after E2.6's join-window fix — `adopt.py` lowers them. | S each | each repo's `.coast/standards-version` at head; its first push green |
...
| E6.3 | **DONE 2026-09-08 (release 1.1.0).** The block title reads "Coast Standards — the standing rules, enforced by machines where a machine can hold them"; the corpus stamp and the block title carry the product name (`coast-rules-version` already does). | S | verify_rules green |
