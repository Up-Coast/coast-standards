...
- **The warnings baseline (built 2026-09-04 at E2.3/E2.4, decision 2 applied
  ...
  in counting mode (`pre-push --measure build,format`, which prints
  `MEASURE <id> <count>` instead of judging) and records the build's
  distinct warnings as `build-warnings` and the formatter's findings as
  `format-findings` in the same `.coast/ratchet-baseline.json`, with the
  same 90-day deadline. From then on the seat runs the tool without its
  ...
  strict (`-warnings-as-errors`, `swift-format lint --strict`), so a fresh
...
| E1.5 parity | Result of the first parity run, 2026-09-04: PENDING — the run is in flight in a clean Coast worktree at 326ca32 (the main checkout is mid-merge on another lane); this row is rewritten with the numbers when it lands. | — | `DocCommentParityTests` |
