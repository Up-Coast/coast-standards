...
| E2.4 | **IN PROGRESS (2026-09-04 evening, Fable).** `adopt.py --dry-run` on keto-tracker lists 22 files; adopt.py now also installs the Claude session hooks, and the pre-push iOS seat builds Xcode projects through `xcodebuild` (commit d2c51ee). Before the real adoption each app is being measured against the seats it will meet (swiftlint --strict with the seed, swift-format --strict, the tree scan, an xcodebuild build) so the first push after adoption is not refused wholesale. The intended handling of legacy findings, per decision 2: ratchets carry baselines automatically; `second-theme-file` is answered by binding each app's own theme file in `.coast/paths.json` (keto: `KetoTracker/DesignSystem/Theme.swift`; Neori: `Neori/Core/DesignSystem/Theme.swift`); lint seeds that fail on legacy code get the failing rules disabled with a "THE RATCHET" note and count (Coast's own pattern), never `--no-verify`; the pre-push `doc-comments --all` seat is to become added-lines-only (the scanner already runs the doc-comments built-in on the diff), because a tree-wide block on a legacy repo is a rewrite day nobody sanctioned. Then commit + push in each app repo through its new hook, record the enforced/total per app here. | S each | adopt.py's own verification |
