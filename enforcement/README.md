...
| E2.3 | **IN PROGRESS (2026-09-04 evening, Fable).** Waits on two things: (1) the E1.5 parity test `Tests/CoastOrchestratorTests/DocCommentParityTests.swift` — written, sitting UNTRACKED in the Coast main checkout because another session holds that checkout mid-merge (lane L); a copy is being run in the clean worktree `Code/coast-workspaces/coast-parity` (detached at 326ca32; first run failed only on the script lookup path, fixed; rerun in flight) — commit the test on Coast main once the other session's merge lands, remove the worktree; (2) the adoption itself: `adopt.py <coast> --platform macos`, then Coast's own `.githooks/pre-push` gains the rules scan seat, `.coast/ratchet-baseline.json` committed (spacing-literal 2,814 · pii-in-log · inline-comment counts), the copy guard retired only after the parity row shows zero difference. | M | Coast's suite green; scanner findings on Coast's Views == the copy guard's |
...
(not itemised on a primary source). Built so far: E0.1–E1.4, E1.7 and E2.2 (section 6 marks
